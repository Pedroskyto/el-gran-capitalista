...
export default function BusinessEmpire() {
  ...
  const save = useCallback(() => {
    try {
      const data = {
        ...
        stocks, cryptoWallet, cryptos, casinoDailyWagered, casinoDailyDate,
  ...
  // Auto-refresh del leaderboard cada 60 segundos si está visible
  useEffect(() => {
    const id = setInterval(() => {
      if (globalLoaded) fetchGlobalLeaderboard();
      if (regionalLoaded) fetchRegionalLeaderboard();
    }, 60000);
    return () => clearInterval(id);
  }, [globalLoaded, regionalLoaded]);
  ...
  const css = `
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .tab-btn { background:none; border:none; color:#9090b8; cursor:pointer; padding:10px 16px; font-family:inherit; font-size:12px; letter-spacing:1px; text-transform:uppercase; border-bottom:2px solid transparent; transition:color .15s,border-color .15s; white-space:nowrap; }
  ...
  return (
    <div style={{ fontFamily:"'Inter',sans-serif", background:"#0c0c1a", color:"#e8e0d0", minHeight:"100vh", display:"flex", flexDirection:"column", userSelect:"none", zoom: uiScale }}>
      ...
      {/* TOP BAR */}
      <div style={{ background:"#0c0c16", borderBottom:"1px solid #161628", padding:"10px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10 }}>
        <div>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        {/* LEFT PANEL */}
        <div style={{ width:232, borderRight:"1px solid #161628", padding:"16px 14px", display:"flex", flexDirection:"column", alignItems:"center", gap:14, overflowY:"auto" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          {/* TABS */}
          <div style={{ borderBottom:"1px solid #161628", padding:"0 12px", display:"flex", gap:0, overflowX:"auto" }}>
            {[["businesses",t.tabBusinesses],["upgrades",t.tabUpgrades],["managers",t.tabManagers],["missions",t.tabMissions||"Missions"],["stocks",t.tabStocks],["crypto",t.tabCrypto||"Crypto"],["portfolio",t.tabPortfolio],["casino",t.tabCasino||"Casino"],["achievements",t.tabAchievements]].map(([id,label]) => (
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"16px 20px", ...(activeTab==="casino" && { backgroundImage:"url('./casino_felt.png')", backgroundRepeat:"repeat", backgroundSize:"220px 220px" }) }}>
                  ...
                  {cryptos.map(c => {
                    ...
                    return (
                      <div key={c.id} style={{ background:"#0c0c18", border:"1px solid #1c1c2e", borderRadius:10, padding:"14px 16px", marginBottom:10 }}>
                        ...
                        {owned > 0 && (() => {
                          const avgBuy   = c.history?.length > 0 ? c.history[0] : c.price;
            ...
            {activeTab === "achievements" && (
              <div>
                {/* Subtabs: Logros | Ranking */}
                {(()=>{ return (
                <div>
                  <div style={{ display:"flex", gap:6, marginBottom:14 }}>
                    {[
                      ["ach",    "🏆 " + (t.achievementsTitle||"Logros")],
                      ["global", "🌍 " + (lang==="es"?"Ranking":lang==="fr"?"Classement":lang==="de"?"Rangliste":lang==="pt"?"Ranking":lang==="ru"?"Рейтинг":"Ranking")]
                    ].map(([id, label]) => (
                      <button key={id} onClick={()=>{ setAchSubTab(id); if(id==="global") { fetchGlobalLeaderboard(); fetchRegionalLeaderboard(); } }}
                        style={{ flex:1, padding:"7px 0", fontSize:11, letterSpacing:.5, cursor:"pointer",
                          fontFamily:"inherit", fontWeight:600, borderRadius:6,
                          background:achSubTab===id?"#1a1530":"#0a0a12",
                          border:"2px solid " + (achSubTab===id?"#8060ff":"#1e1e30"),
                          color:achSubTab===id?"#c0a0ff":"#5555a0" }}>
                        {label}
                      </button>
                    ))}
                  </div>
      ...
      {offlineEarnings && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.72)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"linear-gradient(135deg,#13131f,#1a1a2e)", border:"2px solid #c9a84c", borderRadius:16, padding:"36px 40px", maxWidth:380, width:"90%", textAlign:"center", boxShadow:"0 0 60px rgba(201,168,76,.25)" }}>
            ...
            <div style={{ fontSize:20, fontWeight:700, color:"#c9a84c", marginBottom:4 }}>{t.offlineTitle||"Welcome back!"}</div>
            <div style={{ fontSize:13, color:"#8888aa", marginBottom:20 }}>
              {t.offlineSub||"While you were away"} · {offlineEarnings.seconds >= 3600
                ? Math.floor(offlineEarnings.seconds/3600) + "h " + Math.floor((offlineEarnings.seconds%3600)/60) + "m"
                : Math.floor(offlineEarnings.seconds/60) + "m"}
            </div>
            <div style={{ background:"#0c0c18", border:"1px solid #2a2a40", borderRadius:10, padding:"18px 24px", marginBottom:24 }}>
              <div style={{ fontSize:12, color:"#6a6a8a", letterSpacing:1, marginBottom:4 }}>{t.offlineLabel||"OFFLINE EARNINGS"}</div>
              <div style={{ fontSize:32, fontWeight:700, color:"#6abf7a" }}>{"+" + fmt(offlineEarnings.earned)}</div>
            </div>
            <button
              onClick={() => setOfflineEarnings(null)}
              style={{ background:"linear-gradient(135deg,#c9a84c,#e8c870)", border:"none", borderRadius:8, color:"#080810", fontFamily:"inherit", fontSize:14, fontWeight:700, padding:"12px 32px", cursor:"pointer", letterSpacing:1 }}
            >{t.offlineCollect||"Collect"}</button>
          </div>
        </div>
      )}


      {/* ── AUTO-SAVE INFO POPUP ── */}
      {showSaveInfo && (
        <div className="modal-overlay" style={{ zIndex:10001 }}>
          <div className="modal" style={{ textAlign:"center", maxWidth:360 }}>
            <div style={{ fontSize:44, marginBottom:12 }}>💾</div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#c9a84c", marginBottom:12 }}>
              {t.saveInfoTitle||"Guardado Automático"}
            </div>
            <div style={{ color:"#8888aa", fontSize:13, lineHeight:1.7, marginBottom:22 }}>
              {t.saveInfoBody||"El juego se guarda automáticamente cada 5 minutos y al salir por Opciones → Guardar y salir. No perderás ningún progreso."}
            </div>
            <button onClick={() => setShowSaveInfo(false)} className="btn-pri"
              style={{ padding:"11px 36px", fontSize:14, letterSpacing:.5 }}>
              {t.saveInfoOk||"¡Entendido! A jugar 🚀"}
            </button>
          </div>
        </div>
      )}

      {/* ── TUTORIAL (floating, non-blocking) ── */}
      {tutorialStep > 0 && (() => {
        const endTutorial = () => { setTutorialStep(0); setTutorialDone(true); };
        const TOTAL_STEPS = 8;
        const nextStep    = () => tutorialStep >= TOTAL_STEPS ? endTutorial() : setTutorialStep(s => s + 1);
        const steps = [
          { icon:"🏛️", title: t.tut1t||"Welcome!", body: t.tut1b||"Build your empire from scratch and become the world's richest CEO." },
          { icon:"👆", title: t.tut2t||"Click to earn money", body: t.tut2b||"Click the big gold button on the left to earn your first dollars. The more you click, the faster you grow!" },
          { icon:"🏭", title: t.tut3t||"Buy businesses", body: t.tut3b||"Open the Businesses tab. Each business generates passive income every second — even when you're not clicking!" },
          { icon:"⚙️", title: t.tut4t||"Hire managers", body: t.tut4b||"Managers automate your businesses so they earn money without clicking. They also multiply income up to ×30!" },
          { icon:"⬆️", title: t.tut5t||"Buy upgrades", body: t.tut5b||"The Upgrades tab has permanent multipliers that massively boost your click value and passive income." },
          { icon:"📈", title: t.tut6t||"Invest in stocks & crypto", body: t.tut6b||"Open the Market tab to buy shares and earn dividends every 5 minutes. Crypto fluctuates wildly — big risk, big reward!" },
          { icon:"🎯", title: t.tut7t||"Complete missions", body: t.tut7b||"The Missions tab has Progress, Daily and Chain missions. Complete them for powerful temporary bonuses!" },
          { icon:"✦",  title: t.tut8t||"Prestige to multiply everything", body: t.tut8b||"When you accumulate enough money, Prestige resets your progress but gives a permanent income multiplier. The more you prestige, the more powerful you become!" },
        ];
        const step = steps[tutorialStep - 1];
        return (
          <div style={{
            position:"fixed", bottom:24, right:24, zIndex:9999,
            width:280, background:"#0e0e1a", border:"1px solid #c9a84c",
            borderRadius:14, padding:"18px 18px 14px", boxShadow:"0 8px 32px rgba(0,0,0,0.7)",
            ...
            <div style={{ display:"flex", gap:5, marginBottom:12, justifyContent:"center" }}>
              {Array.from({length:TOTAL_STEPS},(_,i)=>i+1).map(i => (
                <div key={i} style={{ width:7, height:7, borderRadius:4,
                  background: i===tutorialStep ? "#c9a84c" : i<tutorialStep ? "#3a6a2a" : "#2a2a40",
                  transition:"background .2s" }}/>
              ))}
            </div>
            {/* Step counter */}
            <div style={{ fontSize:10, color:"#6060a0", textAlign:"center", marginBottom:8, letterSpacing:1 }}>
            ...
            <div style={{ display:"flex", gap:12, alignItems:"flex-start", marginBottom:14 }}>
              ...
              <div>
                <div style={{ fontSize:13, fontWeight:700, color:"#c9a84c", marginBottom:6 }}>{step.title}</div>
                <div style={{ fontSize:12, color:"#c8c0e0", lineHeight:1.7 }}>{step.body}</div>
              </div>
            </div>
            {/* Buttons */}
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={endTutorial}
                style={{ flex:1, background:"none", border:"1px solid #2a2a40", borderRadius:6,
                  color:"#7070a0", cursor:"pointer", padding:"6px", fontSize:11, fontFamily:"inherit" }}>
                {t.tutSkip||"Skip"}
              </button>
              <button onClick={nextStep}
                style={{ flex:2, background:"linear-gradient(135deg,#1a1810,#2a2215)", border:"1px solid #c9a84c", borderRadius:6,
                  color:"#c9a84c", cursor:"pointer", padding:"8px 10px", fontSize:12,
                  fontFamily:"inherit", fontWeight:700 }}>
                {tutorialStep >= TOTAL_STEPS ? (t.tutFinish||"Let\'s go! 🚀") : (t.tutNext||"Next →")}
              </button>
            </div>
          </div>
        );
